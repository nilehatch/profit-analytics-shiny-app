...
ui <- fluidPage(
  withMathJax(),
  # App title ----
  titlePanel("Profit Analytics - Monopoly"),
  #theme = bs_theme(``
  #  bootswatch = "darkly",
  ...
  fluidRow(
    column(4,
           wellPanel(
             selectInput("wtpCol", "Name of Willingness-to-Pay Column", choices = c("FILE NEEDS TO BE UPLOADED")),
             selectInput("regressType", "Demand Curve Shape", choices = c("Linear", "Exponential", "Sigmoid")),
             numericInput("pop", "Target Customer Population Size", min = 0, value = 1000),
...
server <- function(input, output) {
  ...
  userData <- reactive({
    ...
    if (!all(sapply(df, is.numeric))) {
      stop("The data must contain numeric columns only.")
    }
    # Check for missing values
    if (anyNA(df)) {
      stop("The data contains missing values. Please clean your data.")
    }
    df
  })
...
#  output$contents <- renderDataTable(userData(), options = list(pageLength = 8))
  output$contents <- DT::renderDT({
    validate(
      need(!is.null(userData()), "Please upload a valid file.")
    )
    datatable(userData(), options = list(pageLength = 8))
  })
  ...
  userWTPCol <- reactive({
    req(input$wtpCol != "FILE NEEDS TO BE UPLOADED")
    col <- userData()[[input$wtpCol]]
    col <- as.numeric(col) # Ensure it's numeric
    col <- na.omit(col)    # Remove missing values
    if (length(col) == 0) stop("Selected column is empty or contains invalid values.")
    col
  })
  ...
  observeEvent(input$wtpCol, {
    col <- userWTPCol()
    if (length(col) > 0) {
      updateSliderInput(inputId = "price",
  ...
  output$demand_Plot_fit <- renderPlot({
    validate(
      need(!is.null(userCleanData()), "Cleaned data is missing or invalid."),
      need(length(userWTPCol()) > 0, "WTP column is empty or invalid.")
    )
